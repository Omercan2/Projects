...
void adminFunction(){
    ...
    //ogretim gorevlilsi gorevlendirme
    for(int i=0;i<4;i++){
...
void officerFunction(){
    ...
    scanf("%d",&choice);
    if(choice==1){ //ogrenci ekleme
        addStudent();

    }
    else if(choice==2){  //ders acma
        addLesson();
    }
...
int main()
{
    ...
    student[0].number=-1;//ogrencinin numarasi -1 ise orasi bos anlamina geliyor
    login();
